% % ==========================================================
% % function_Recon_RC.m (V5 - 支持多角度集合)
% % ==========================================================
...
% 提取 行发(1:row_waves) 列收(129:256) 的数据
RF_data = permute(channel_RF(:,129:256,1:row_waves,1), [1 3 2]); % [1920x33x128]
...
if strcmpi(angle_set, 'full')
    angle_indices = 1:row_waves; % 1:33
    disp(' [Function] 开始 RC 重建循环 (Full 33 Angles)...');
elseif strcmpi(angle_set, 'zero')
    angle_indices = 17; % 仅 0 度角
    disp(' [Function] 开始 RC 重建循环 (Zero Angle Only)...');
else
...
recon_time = toc;
fprintf(' [Function] RC 循环 (set: %s) 完成, 耗时: %.2f 秒\n', angle_set, recon_time);
